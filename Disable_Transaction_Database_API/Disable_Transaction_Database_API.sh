...
output_json=/opt/ProvisioningAPI_2022/Disable_Transaction_Database_API/output.json
destn_invent=/opt/ProvisioningAPI_2022/Disable_Transaction_Database_API/inventory.txt
...
ansible-playbook /opt/ProvisioningAPI_2022/Disable_Transaction_Database_API/Disable_Transaction_Database_API.yml -i $destn_invent --extra-vars="customer_name=$input_customer lob_name=$input_lob target_host=$target_hostnames port_num=$target_port"
if [[ $? != 0 ]]
then
echo "Disable TransactionDB Playbook execution got failed"
exit 1
fi
echo "=============== END OF PLAY ============================="
}
